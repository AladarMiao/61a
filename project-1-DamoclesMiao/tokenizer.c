  ...
  */
  ...
  if (buffer[size_read] == '+') {  // + and ++
    ...
    if (buffer[size_read+1]=='/') {
        ...
        IS_COMMENT=1;
        size_read++;
        while(size_read+1<size&&IS_COMMENT){
            if(buffer[size_read+1]=='\n'){IS_COMMENT=0;(*linenum)++;}
            size_read++;
        }
        return size_read;
      /* YOUR CODE HERE*/
    ...
    if (size_read + str_len >= size) {
      ...
      int char_len=1;
      if(replace_escape_in_character(buffer+size_read+char_len)!=-1){
        char_len+=2;

      }

      else if(isprint(buffer[size_read+char_len])){

         char_len+=1;

     }
           if(buffer[size_read+char_len]=='\'' && buffer[size_read+1]!='\'' && buffer[size_read+1]!='\\'){
    ...
    /* FIXME IM NOT CORRECT. */
             ...
             buffer[size_read] == '"') {  // strings and some errors
    size_t str_len = 1;
    int search = 1;
    while (size_read + str_len < size && search) {
      if (buffer[size_read + str_len] == '\\') {
    ...
    if(buffer[size_read]=='0' && is_digit(buffer[size_read+1])){
    ...
    else if((buffer[size_read]=='0')&&!is_digit(buffer[size_read+1])){
        t = create_token(filename);
        t->linenum = *linenum;
        ...
        t = create_token(filename);
                ...
                t->type = TOKEN_INTEGER;
                char *ptr;
                t->data.integer = (int) strtol(buffer+size_read, &ptr, 10);
                size_read=ptr-buffer;
        ...
        if (type != TOKEN_ERR) {
          ...
          size_read += id_len;
            ...
            int i = 0;
                     ...
                     t->data.identifier=(char*)malloc(sizeof(char) * strlen(token_contents) + 1);
                     while(token_contents[i]!='\0'){
